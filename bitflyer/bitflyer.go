...
// Order System
// Link: https://lightning.bitflyer.com/docs#send-a-new-order
type Order struct {
	ID                     int     `json:"id"`
...
type ResponseSendChildOrder struct {
	ChildOdrderAcceptanceID string `json:"child_order_acceptance_id"`
}
...
func (api *APIClient) ListOrder(query map[string]string) ([]Order, error) {
	...
	if err != nil {
		log.Printf("action=ListOrder, err=%s", err.Error())
	}
