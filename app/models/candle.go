...
func (c *Candle) TableName() string {
	return GetTableName(c.ProductCode, c.Duration)
}

// CREATE
func (c *Candle) Create() error {
	cmd := fmt.Sprintf("INSERT INTO %s (time, open, close, high, low, volume) VALUE (?, ?, ?, ?, ?, ?, ?)", c.TableName())
...
func GetCandle(productCode string, duration time.Duration, dateTime time.Time) *Candle {
	tableName := GetTableName(productCode, duration)
	...
	if err != nil {
		log.Printf("action=GetCandle, err=%s", err.Error())
		return nil
	}
...
func CreateCandleWithDuration(ticker bitflyer.Ticker, productCode string, duration time.Duration) bool {
	...
	// If it's the first time to get price, all will be the first result
